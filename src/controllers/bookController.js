...
const getBooksInYear = async function(req,res)
{
    ...
    res.send({msg:getdatabyyear})
}

const getParticularBooks = async function(req,res)
{
    // let allBooks= await BookModel.find( { bookName:  /.*hi.*/i  }) 
    //  res.send(allBooks)
...
const getXINRBooks=  async function(req,res){
    ...
    let getXINRBooks = await BookModel.find({"prices.indianPrice":{$in:["Rs100","Rs200","Rs300"]}})
    res.send({msg:getXINRBooks})
}

const randombooks = async function(req,res){
    let randombooks = await BookModel.find({$or:[{totalPages:{$gte:350}},{stockAvailable:true}]})
     res.send({msg:randombooks})
}
    // get year
    // let year=req.query.year
    // let getdatabyyear = await BookModel.find({year:{$eq:year}})
    // res.send(getdatabyyear)
    ...
    //●	getXINRBooks 
    // let data=req.body.prices.indianPrice
    // let getXINRBooks = await BookModel.find({indianPrice:{$in:["Rs100","Rs200","Rs350"]}})
